/**
 * Pure TypeScript implementation of the MD5 hashing algorithm.
 * Conforms to RFC 1321. Dependency-free.
 */
export function generateMd5(string: string): string {
...
/**
 * Computes standard SHA hashes asynchronously using standard Web Crypto API.
 */
export async function generateSha(str: string, algorithm: 'SHA-1' | 'SHA-256' | 'SHA-512'): Promise<string> {
  if (!str) return '';
  ...
  const hashBuffer = await cryptoObj.subtle.digest(algorithm, data);
